...
public class tec {
    public static void InsertData(propdoc p)
    {
        try
        {
         ...
         PreparedStatement stmt=con.prepareStatement("insert into tec (dname,dob,gender,qual,sid,ssid,email,contact,address,hid,login,password)values(?,?,?,?,?,?,?,?,?,?,?,?)");
     ...
     public static void deletedata(propdoc p)
    {
        ...
        {
         Connection con=ConnectionProvider.getcon();
         PreparedStatement stmt=con.prepareStatement("delete from teachers where dname=?");
         stmt.setString(1, p.getdName());
         stmt.executeUpdate();
        }
        catch(Exception e)
   ...
   public static void updateData(propdoc p)
    {
        ...
        {
         Connection con=ConnectionProvider.getcon();
         PreparedStatement stmt=con.prepareStatement("update teachers set dob=?,gender=?,qual=?,sid=?,ssid=?,email=?,contact=?,address=?,hid=?,login=?,password=? where dname=?");
    ...
    public static ResultSet getLogin(propdoc p)
    {
            ...
            try
        {
         Connection con=ConnectionProvider.getcon();
         Statement stmt=con.createStatement();
        rs=stmt.executeQuery("select login,password from teachers where login='"+p.getLogin()+"'");
         }
                catch(Exception e)
        {}
            return rs;
 ...
 public static ResultSet selectData(String n)
    ...
    ResultSet rs=null;
            try
        {
         Connection con=ConnectionProvider.getcon();
         Statement stmt=con.createStatement();
        rs=stmt.executeQuery("select * from teachers where login='"+n+"'");
         }
                catch(Exception e)
        {}
            return rs;
